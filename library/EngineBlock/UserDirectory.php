...
class EngineBlock_UserDirectory 
{
    ...
    public function getUser($name) {
        ...
        $filter .= '(' . ENGINEBLOCK_LDAP_ATTR_COLLAB_PERSON_ID . '=' . $name . '))';
        $collection = $this->_getLdapClient()->search($filter);
        $result = FALSE;
    ...
    private function getCommonNameFromAttributes($attrs) {
	    $result = NULL;
    	if ( (array_key_exists('givenName', $attrs)) and (array_key_exists('sn', $attrs)) ) {
	    	$result = $attrs['givenName'][0] . ' ' . $attrs['sn'][0];
	    } else if (array_key_exists('sn', $attrs)) {
		    $result = $attrs['sn'][0];
       	} else if (array_key_exists('displayName', $attrs)) {
	    	$result = $attrs['displayName'][0];
	    } else if (array_key_exists('mail', $attrs)) {
    		$result = $attrs['mail'][0];
	    } else if (array_key_exists('givenName', $attrs)) {
    ...
    public function addOrganization($organization) {
		$info = array(
			'o' => $organization,
			'objectclass' => array('organization', 'top'),
		);
    ...
    public function addUser($organization, $attributes, $attributeHash) {
        ...
        foreach (array('uid', 'cn', 'sn', 'mail', 'displayName', 'givenName') as $attribute) {
        ...
        if (!array_key_exists('cn', $info)) $info['cn'] = $this->getCommonNameFromAttributes($info);
        if (!array_key_exists('sn', $info)) $info['sn'] = $info['cn'];
        
        $info['objectClass'] = array('collabPerson', 'nlEduPerson', 'inetOrgPerson', 'organizationalPerson', 'person', 'top');
        $info['o'] = $organization;
