...
/**
 * This is a Fork of SimpleSAMLphps Message class used soley
 ...
 * @version $Id$
 */
class EngineBlock_Ssp_sspmod_saml_Message
{
    /**
     ...
     */
    public static function addSign(SimpleSAML_Configuration $srcMetadata, SimpleSAML_Configuration $dstMetadata, SignedElement $element)
    {
     ...
     */
    private static function addRedirectSign(SimpleSAML_Configuration $srcMetadata, SimpleSAML_Configuration $dstMetadata, Message $message)
    {
        $signingEnabled = null;
     ...
     *
     * @param SimpleSAML_Configuration $srcMetadata  The metadata of the sender.
     * @param SignedElement $element  Either a \SAML2\Response or a \SAML2\Assertion.
     */
    ...
    private static function processAssertion(
        SimpleSAML_Configuration $spMetadata, SimpleSAML_Configuration $idpMetadata,
        Response $response, $assertion, $responseSigned)
    {
        assert('$assertion instanceof \SAML2\Assertion || $assertion instanceof \SAML2\EncryptedAssertion');
        ...
        $timeProvider = EngineBlock_ApplicationSingleton::getInstance()->getDiContainer()->getTimeProvider();

        $notBefore = $assertion->getNotBefore();
        if ($notBefore !== null && $notBefore > $timeProvider->time() + 60) {
        ...
        if ($notOnOrAfter !== null && $notOnOrAfter <= $timeProvider->time() - 60) {
        ...
        if ($sessionNotOnOrAfter !== null && $sessionNotOnOrAfter <= $timeProvider->time() - 60) {
        ...
        foreach ($assertion->getSubjectConfirmation() as $sc) {
            ...
            if ($scd->NotBefore && $scd->NotBefore > $timeProvider->time() + 60) {
                $lastError = 'NotBefore in SubjectConfirmationData is in the future: ' . $scd->NotBefore;
                continue;
            }
            if ($scd->NotOnOrAfter && $scd->NotOnOrAfter <= $timeProvider->time() - 60) {
