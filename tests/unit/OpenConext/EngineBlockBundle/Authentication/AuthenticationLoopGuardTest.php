...
class AuthenticationLoopGuardTest extends TestCase
{
    ...
    public function authentication_loop_guard_determines_that_it_is_in_a_loop()
    {
        ...
        $secondProcedure  = AuthenticationProcedure::onBehalfOf($serviceProvider);
        $secondProcedure->authenticateAt($identityProvider);
        $secondProcedure->completeOn(new DateTimeImmutable());

        $pastAuthenticationProcedures = new AuthenticationProcedureList([$firstProcedure, $secondProcedure]);
