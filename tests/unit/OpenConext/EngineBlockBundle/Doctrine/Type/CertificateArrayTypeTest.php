...
class CertificateArrayTypeTest extends TestCase
{
    ...
    #[Group('Doctrine')]
    #[Test]
    public function a_null_or_empty_array_is_null_conversion()
    {
        $certType = Type::getType(CertificateArrayType::NAME);

        $value = $certType->convertToDatabaseValue(null, $this->platform);
        $emptyArray = $certType->convertToDatabaseValue([], $this->platform);

        $this->assertNull($value);
        $this->assertNull($emptyArray);
    }
    ...
    #[Group('Doctrine')]
    #[Test]
    public function a_null_value_is_converted_to_empty_array()
    {
        $certType = Type::getType(CertificateArrayType::NAME);
    ...
    public function saved_object_equals_result()
    {
        ...
        // Verify that the certificate is actually valid.
        $factory = new X509CertificateFactory();
        // baseline
        $this->assertNotFalse(openssl_x509_read($factory->fromCertData($this->certData)->toPem()));
        // converted
        $this->assertNotFalse(openssl_x509_read($factory->fromCertData($value[0]->toCertData())->toPem()));
    }
    ...
    public function an_invalid_php_value_causes_an_exception_upon_conversion()
    {
        ...
        $this->expectExceptionMessage('Value "" must be null or an instance of engineblock_certificate_array (null, array) to be able to convert it to a database value');

        $certType->convertToDatabaseValue("", $this->platform);
    }
    ...
    public function an_invalid_database_value_causes_an_exception_upon_conversion()
    {
        ...
        $this->expectException(ConversionException::class);
        $this->expectExceptionMessage('Could not convert database value "" to Doctrine Type engineblock_certificate_array. Expected format: X509Certificate');
