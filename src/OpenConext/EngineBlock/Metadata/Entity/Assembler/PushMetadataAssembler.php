...
class PushMetadataAssembler implements MetadataAssemblerInterface
{
     ...
     * @Deprecated
     */
    public function assembleEb5($connections)
    {
        $roles = array();
        $allIdpEntityIds = array();
        $spAllowedEntityIds = array();
        ...
        foreach ($connections as $connection) {
            $role = $this->assembleConnectionEb5($connection);
     ...
     * @Deprecated
     */
    private function assembleConnectionEb5(stdClass $connection)
    {
        if ($connection->type === 'saml20-sp') {
            return $this->assembleSpEb5($connection);
        }

        if ($connection->type === 'saml20-idp') {
            return $this->assembleIdpEb5($connection);
        }
     ...
     * @Deprecated
     */
    private function assembleSpEb5(stdClass $connection)
    {
        $properties = $this->assembleCommon($connection);
     ...
     * @Deprecated
     */
    private function assembleIdpEb5(stdClass $connection)
    {
        $properties = $this->assembleCommon($connection);
