...
class AttributeReleasePolicyType extends Type
{
    ...
    public function convertToPHPValue($value, AbstractPlatform $platform): ?AttributeReleasePolicy
    {
        ...
        $unserialized = unserialize($value, ['allowed_classes' => [AttributeReleasePolicy::class]]);
        if ($unserialized instanceof AttributeReleasePolicy) {
            return $unserialized;
        }

        throw new ConversionException(sprintf(
            'Invalid format for %s expected serialized AttributeReleasePolicy, got: %s',
            $this->getName(),
