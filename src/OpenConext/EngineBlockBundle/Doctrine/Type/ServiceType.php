...
class ServiceType extends Type
{
    ...
    public function convertToPHPValue($value, AbstractPlatform $platform): ?Service
    {
        ...
        $unserialized = unserialize($value, ['allowed_classes' => [Service::class, IndexedService::class]]);
        if ($unserialized instanceof Service) {
            return $unserialized;
        }

        throw new ConversionException(sprintf(
            'Invalid format for %s expected serialized Service, got: %s',
            $this->getName(),
