...
class Wayf extends Twig_Extension
{
        ...
        return array_filter(
            array_map(
                function (array $idp) use ($previousSelectionIndex) {
                    $entryKey = $this->entryKey($idp['entityId'], $idp['discoveryHash']);
    ...
    public function entryKey(string $entityId, ?string $discoveryHash = null): string
    {
        return (new IdpHistoryService())->makeEntryKey($entityId, $discoveryHash);
    }
}
